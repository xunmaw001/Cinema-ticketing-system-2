...
public class DianyingOrderController {
    ...
    @Autowired
    private HuodongService huodongService;//活动信息
    @Autowired
...
//                            dianyingOrderEntity.setChangciTypes(Integer.valueOf(data.get(0)));   //场次 要改的
    ...
    public R add(@RequestBody DianyingOrderEntity dianyingOrder, HttpServletRequest request){
        logger.debug("add方法:,,Controller:{},,dianyingOrder:{}",this.getClass().getName(),dianyingOrder.toString());
            ...
            List<DianyingOrderEntity> dianyingOrderEntityList =
                        ...
                        .eq("changci_types", dianyingOrder.getChangciTypes())
        ...
        if(dictionaryEntity != null ){
            ...
            if(dianyingOrder.getDianyingOrderPaymentTypes()==1){//余额支付
                double balance = yonghuEntity.getNewMoney() - dianyingEntity.getDianyingNewMoney()*(dianyingOrder.getBuyZuoweiNumber().split(",").length)*zhekou;//余额
                if(balance<0)
                    return R.error(511,"余额不够支付");
                dianyingOrder.setDianyingOrderTruePrice(dianyingEntity.getDianyingNewMoney()*(dianyingOrder.getBuyZuoweiNumber().split(",").length)*zhekou); //设置实付价格
                dianyingOrder.setDianyingOrderPaymentTypes(1);
                yonghuEntity.setNewMoney(balance);//设置金额
                int length = dianyingOrder.getBuyZuoweiNumber().split(",").length;
                buyJifen=Double.valueOf(dianyingEntity.getDianyingPrice()*length);
                //更新第一注册表
                ...
                double balance = yonghuEntity.getYonghuNewJifen() - dianyingEntity.getDianyingNewMoney()*(dianyingOrder.getBuyZuoweiNumber().split(",").length);//余额
                if(balance<0)
                    return R.error(511,"积分不够支付此次兑换.");
                ...
                dianyingOrder.setDianyingOrderPaymentTypes(2);
                yonghuEntity.setYonghuNewJifen(balance);//花费积分
            }
    ...
    */
    ...
    public R refund(Integer id, HttpServletRequest request){
        ...
        String role = String.valueOf(request.getSession().getAttribute("role"));
            ...
            Integer dianyingOrderPaymentTypes = 1;
            ...
            if(dianyingOrderPaymentTypes == 1){//余额支付
                ...
                //计算所获得积分
                Double buyJifen = 0.0;
                buyJifen = new BigDecimal(dianyingEntity.getDianyingPrice()).multiply(new BigDecimal(buyNumber)).doubleValue();
                ...
                Double money = dianyingEntity.getDianyingNewMoney() * buyNumber;

...
public class DianyingCommentbackController {
    ...
    private static final String TABLE_NAME = "dianyingCommentback";
    ...
    */
    ...
    public R update(@RequestBody DianyingCommentbackEntity dianyingCommentback, HttpServletRequest request) throws NoSuchFieldException, ClassNotFoundException, IllegalAccessException, InstantiationException {
        logger.debug("update方法:,,Controller:{},,dianyingCommentback:{}",this.getClass().getName(),dianyingCommentback.toString());
        DianyingCommentbackEntity oldDianyingCommentbackEntity = dianyingCommentbackService.selectById(dianyingCommentback.getId());//查询原先数据
...
//        else if("用户".equals(role))
//            dianyingCommentback.setYonghuId(Integer.valueOf(String.valueOf(request.getSession().getAttribute("userId"))));
        if("".equals(dianyingCommentback.getDianyingCommentbackText()) || "null".equals(dianyingCommentback.getDianyingCommentbackText())){
                dianyingCommentback.setDianyingCommentbackText(null);
        }
        if("".equals(dianyingCommentback.getReplyText()) || "null".equals(dianyingCommentback.getReplyText())){
                dianyingCommentback.setReplyText(null);
        }
    ...
    */
    ...
    public R delete(@RequestBody Integer[] ids, HttpServletRequest request){
        ...
        List<DianyingCommentbackEntity> oldDianyingCommentbackList =dianyingCommentbackService.selectBatchIds(Arrays.asList(ids));//要删除的数据
...
//                            dianyingCommentbackEntity.setDianyingCommentbackPingfenNumber(Integer.valueOf(data.get(0)));   //评分 要改的
...
//                            dianyingCommentbackEntity.setUpdateTime(sdf.parse(data.get(0)));          //回复时间 要改的

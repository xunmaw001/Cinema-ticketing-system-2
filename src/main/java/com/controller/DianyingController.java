    ...
    */
    ...
    public R save(@RequestBody DianyingEntity dianying, HttpServletRequest request){
        ...
        Wrapper<DianyingEntity> queryWrapper = new EntityWrapper<DianyingEntity>()
            ...
            .eq("shangying_time", new SimpleDateFormat("yyyy-MM-dd").format(dianying.getShangyingTime()))
            .eq("dianying_zhuyan", dianying.getDianyingZhuyan())
            .eq("dianying_diqu", dianying.getDianyingDiqu())
            .eq("dianying_yuyan", dianying.getDianyingYuyan())
            .eq("dianying_didian", dianying.getDianyingDidian())
            .eq("zan_number", dianying.getZanNumber())
            .eq("cai_number", dianying.getCaiNumber())
            .eq("dianying_price", dianying.getDianyingPrice())
            .eq("zuowei_number", dianying.getZuoweiNumber())
            .eq("dianying_shichang", dianying.getDianyingShichang())
...
//                            dianyingEntity.setShangyingTime(sdf.parse(data.get(0)));          //上映日期 要改的
//                            dianyingEntity.setDianyingZhuyan(data.get(0));                    //主演 要改的
//                            dianyingEntity.setDianyingDiqu(data.get(0));                    //电影地区 要改的
//                            dianyingEntity.setDianyingYuyan(data.get(0));                    //语言 要改的
//                            dianyingEntity.setDianyingDidian(data.get(0));                    //放映地点 要改的
//                            dianyingEntity.setZanNumber(Integer.valueOf(data.get(0)));   //赞 要改的
//                            dianyingEntity.setCaiNumber(Integer.valueOf(data.get(0)));   //踩 要改的
//                            dianyingEntity.setDianyingPrice(Integer.valueOf(data.get(0)));   //购买获得积分 要改的
//                            dianyingEntity.setDianyingOldMoney(data.get(0));                    //电影原价 要改的
//                            dianyingEntity.setDianyingNewMoney(data.get(0));                    //现价/积分 要改的
//                            dianyingEntity.setZuoweiNumber(Integer.valueOf(data.get(0)));   //座位 要改的
//                            dianyingEntity.setDianyingShichang(Integer.valueOf(data.get(0)));   //电影时长 要改的
//                            dianyingEntity.setDianyingClicknum(Integer.valueOf(data.get(0)));   //电影热度 要改的
